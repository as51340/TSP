...
"""
Self-adaptivity is kind of parameter tuning when parameters are added to solution representation.
Different representations are very often used.
TODO look at Chapter 4 next week, code is seen. 	 
"""
...
class eggholderEA:
	...
	def optimize( self, plotFun = lambda x : None):
		...
		# random samples from uniform distribution withih 0 and 1
		...
		for i in range(self.numIters):
			...
			offspring = self.mean_crossover(selected)  # try mean crossover, it shouldn't work too well.
	...
	"""
	Recombination using mean - not great crossover because it rapidly converges. Larger k means larger selective pressure which again isn't great because of bad space exploration. 
	Large amount of times solution is someway between optimal and local minimum.
	"""
	def mean_crossover(self, selected):
		offspring = np.zeros((self.lambdaa, 2))  # offspring has same size as population.
		for ii, _ in enumerate(offspring):
			offspring[ii, :] = np.clip((selected[2 * ii, :] + selected[2 * ii + 1, :]) / 2.0, 0, self.intMax)
		return offspring
	...
	"""
	Strongly explorative mutation where you want to every time reset number that is within some boundaries.
	"""
	def resetting_mutation(self, offspring, alpha):
		ii = np.where(np.random.rand(np.size(offspring, 0)) <= alpha)  # precompute alpha values
		a = np.random.rand(0, self.intMax)
		b = np.random.rand(0, self.intMax)
		if a < b:
			offspring[ii, :] = (np.random.rand(np.size(offspring, 0)) / (b - a)) + a
		else:
			offspring[ii, :] = (np.random.rand(np.size(offspring, 0)) / (a - b)) + b
...
def plotPopulation2D(input):
	...
	X, Y = np.meshgrid(x, y)
	stck = np.vstack((X.flatten(),Y.flatten()))
	Z = myfun(np.transpose(stck))
	...
	ax = fig.gca()
	ax.imshow(Z)
	ax.scatter(population[:,0], population[:,1], marker='o', color='r')  # our minimum
	ax.scatter(mini, minj, marker='*', color='yellow')  # real minimum
	plt.pause(0.05)
